...
class Vector{
    ...
    copy()
    {
        let vector = new Vector(this.values.length);
        let a = [];
        // console.log(this.values)
...
class NeuralNetwork
{
    ...
    // LoadValues from JSON 
    loadFromJSON(json)
    {
 
        let i_hidden   =   json.I_hidden;
        let I_input    =   json.I_input;
        let I_output   =   json.I_output;
        let cross      = json.crossOverMethod;
        let mut        = json.mutationRate;

        let child =  new NeuralNetwork(I_input,i_hidden,I_output,cross,mut);

        for(let i = 0 ; i < child.allLayersArray.length; i++)
        {
            child.WeightMatrices[i].values = json.allLayersArray[i].weightMatrix.values;
            child.Biases[i].values         = json.allLayersArray[i].bias.values;
        }
