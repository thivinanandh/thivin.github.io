
/// The global Variables

let N
let Mutation_Rate
let maxPopulation
let chessBoard;

let queenImage;
let Start = false;

let populationObject;
let myCanvas;

let userSelectedCrossOver;

function preload() {
    queenImage = loadImage('data:image/png;base64,');
}

function starter()
{
    Start = true;
    setupCommands();
    loop();
    
}

function setupCommands()
{
    userSelectedCrossOver = document.getElementById("crossover").value;
    N = document.getElementById("nSlider").value;
    Mutation_Rate = document.getElementById("mutationSlider").value;;
    maxPopulation = document.getElementById("populationSlider").value;


    // Print Actual Phrase 
    let winWidth = 400;
    let winHeight = 400; 
    myCanvas = createCanvas(winWidth, winHeight);
    
    myCanvas.parent("Canvas");

    // Load the queen 
    // image(queenImage, 0, 0);

    // Create a Chess Board object 
    chessBoard =  new Chess(N,myCanvas,queenImage)

    populationObject = new population(Mutation_Rate,maxPopulation,chessBoard);
    
    document.getElementById("Fitness").innerHTML = 0
    document.getElementById("generation").innerHTML = 00;
    

}

function setup()
{ 
    setupCommands()
    
}


function draw()
{
    
    if(Start)
    {
        // Perform Selection
        populationObject.selection();

        // compute Fitness 
        populationObject.computeFitness();


        if(populationObject.isMaxFittnessReached())
        {
            noLoop();
        }

        updateInfo();
    }
        

}


function updateInfo()
{
    document.getElementById("generation").innerHTML = populationObject.generationCount;
    document.getElementById("Mutation").innerHTML = Mutation_Rate;
    let globalMax = N*(N-1);
    let currentMax = populationObject.bestGeneScore;
    let percentage  = (currentMax/globalMax) * 100;
    
    document.getElementById("Fitness").innerHTML    = percentage.toFixed(2) + " %";
    document.getElementById("Population").innerHTML = maxPopulation;
    
    populationObject.getBestGenome()

    // var rightTable = document.getElementById("generations");
    // rightTable.innerHTML = populationObject.getDisplayPhrase();
}